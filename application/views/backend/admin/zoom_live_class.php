...
<div class="row">
    <div class="col-md-6">
        <div class="form-group">
            <label for="zoom_topic">
                <?php echo get_phrase('Meeting Topic'); ?> <span class="required">*</span>
            </label>
            <input value="<?php echo $zoom_meeting['topic'] ?? '' ?>" type="text" class="form-control" id="zoom_topic" placeholder="Introduction Meeting">
        </div>

        <div class="form-group">
            <label for="zoom_duration">
                <?php echo get_phrase('Meeting Duration'); ?> <span class="required">*</span>
            </label>
...
<script>
    ...
    function save_zoom_meeting() {
        ...
        if (zoom_topic == '' || zoom_duration == '') {
            error_notify('<?php echo get_phrase("Meeting topic and duration can not be empty"); ?>');
        } else {
            $.ajax({
                type: 'POST',
